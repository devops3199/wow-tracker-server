...
export class playRepository {
  async findAll() {
    ...
    const getAllPlay = (): Promise<string[]> => {
      return new Promise((resolve, reject) => {
        connection.query(
          `SELECT date, begin, end, dungeon, raid FROM play OREDER BY date)`,
  ...
  async save(play: Play) {
    ...
    const savePlay = (play: Play): Promise<void> => {
      return new Promise((resolve, reject) => {
        connection.query(
          `INSERT INTO play (date, begin, end, dungeon, raid) VALUES (${play.date}, ${play.begin}, ${play.end}, ${play.dungeon}, ${play.raid})`,
          (error, results, fields) => {
            if (error) {
